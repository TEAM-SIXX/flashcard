...
export default function App() {
  ...
  const browseDeck =
    filterTech === "All"
      ? MASTER_QA_DB
      : MASTER_QA_DB.filter((q) => q.tech === filterTech);
...
${text.slice(0, 3000)}`,
        ...
        const allTechs = [
          ...new Set([...aiCards.map((c) => c.tech), ...localTechs]),
        ];
  ...
  const extractFromUrl = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        setError(
          "Could not fetch URL. Paste the job description text directly instead."
        );
        setLoading(false);
        setStatus("");
      }
  ...
  const tabs = [
    ["extract", "⚡ Extract"],
    ["browse", "◈ Browse DB"],
    ["study", deck.length ? `▶ Study (${deck.length})` : "▶ Study"],
  ];
        ...
        <div style={styles.headerLeft}>
          ...
          <div>
            ...
            <div style={styles.logoSub}>
              Interview Prep Engine · v3.0 · Powered by Groq
            </div>
          </div>
        </div>
        <StudyTimer
        ...
        {tab === "extract" && (
          <div style={styles.panel}>
            <div style={styles.offlineBadge}>
              ⚡ Groq AI active — paste any job description. Llama3 extracts
              technologies and generates custom flashcards merged with the master
              Q&amp;A database.
            </div>
            ...
            <button
              style={{
                ...styles.primaryBtn,
                opacity: loading || !inputText ? 0.45 : 1,
              }}
              disabled={loading || !inputText}
              onClick={() => extractFromText(inputText)}
            >
            ...
            <div style={styles.divider}>
              ...
              <span style={styles.dividerText}>or scrape a URL</span>
              <div style={styles.dividerLine} />
            </div>
            ...
            <div style={styles.urlRow}>
              ...
              <button
                style={{
                  ...styles.scrapeBtn,
                  opacity: loading || !inputUrl ? 0.45 : 1,
                }}
            ...
            <div style={styles.freeApis}>
              ...
              <div style={styles.apiRow}>
                <span style={styles.apiName}>Groq (Llama3)</span>
                <span style={styles.apiNote}>
                  Free AI — console.groq.com
                </span>
              </div>
              <div style={styles.apiRow}>
                <span style={styles.apiName}>r.jina.ai</span>
                <span style={styles.apiNote}>
                  Free URL scraper — no key needed
                </span>
              </div>
              <div style={{ ...styles.apiRow, borderBottom: "none" }}>
                <span style={styles.apiName}>api.allorigins.win</span>
                <span style={styles.apiNote}>Fallback CORS proxy — free</span>
              </div>
            </div>
          ...
          <div style={styles.panel}>
            <div style={styles.browseHeader}>
              <div style={styles.sectionTitle}>
                Master Q&amp;A Database — {MASTER_QA_DB.length} Questions
              </div>
              <div style={styles.filterRow}>
                {["All", ...ALL_TECHS].map((t) => (
                  <button
                    ...
                    style={{
                      ...styles.filterChip,
                      ...(filterTech === t ? styles.filterChipActive : {}),
                    }}
                    onClick={() => {
                      setFilterTech(t);
                      setDeckIndex(0);
              ...
              >
                ▶ Study these {browseDeck.length} cards
              </button>
            </div>
            ...
            <div style={styles.dbList}>
              {browseDeck.map((item, i) => (
                <div key={item.id} style={styles.dbRow}>
                  <div style={styles.dbRowLeft}>
                    <span style={styles.dbIndex}>#{i + 1}</span>
                    <span style={styles.dbTech}>{item.tech}</span>
                  </div>
            ...
            {deck.length === 0 ? (
              <div style={styles.emptyState}>
                ...
                <div style={styles.emptySub}>
                  Extract from a job posting or load the full database.
                </div>
                <div style={styles.emptyActions}>
                  ...
                  >
                    ⚡ Extract from job post
                  </button>
                  <button
                    style={styles.ghostBtn}
                    onClick={() => {
                      setDeck(MASTER_QA_DB);
                      setDeckIndex(0);
                    }}
                  >
                    Load all {MASTER_QA_DB.length} cards
                  </button>
                </div>
              </div>
            ) : (
              <Flashcard
                ...
                total={deck.length}
                onNext={() =>
                  setDeckIndex((i) => Math.min(i + 1, deck.length - 1))
                }
